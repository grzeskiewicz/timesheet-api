...
function handleDisconnect() {
  ...
  connection.on("error", function (err) {
    ...
    if (err.code === "PROTOCOL_CONNECTION_LOST") {
      ...
      setTimeout(handleDisconnect, 2000); //test
    }
  });
}
  ...
  connection.query(
    ...
    function (err, rows) {
      ...
      if (userExists) res.json({ success: false, msg: "USER EXISTS" });
        ...
        connection.query(
          `INSERT INTO users(name,surname,email,password,deal,role,creation_date) VALUES("${name}","${surname}","${email}","${password}",${deal},${role},'${creationDate}')`,
          function (err, result) {
...
const editUser = function (req, res) {
  ...
  connection.query(
    `UPDATE IGNORE users SET name='${name}', 
    surname='${surname}', 
...
/*
const updateUserSignature = function (req, res) { //file saved on the server option
...
const updateUserSignature = function (req, res) {
  //file saved on the server option
  const userID = req.body.user;
  const signature = req.body.myImage;
  console.log(userID)
  connection.query(
    `UPDATE IGNORE users SET signature='${signature}' WHERE id='${userID}'`,
    function (err, rows) {
      if (err) res.json(err);
      if (rows.changedRows === 0) {
        res.json({ success: false, msg: "NO CHANGES" });
...
const deleteUser = function (req, res) {
  connection.query(
    "delete from users where id=" + req.body.user,
    function (err, rows) {
...
const users = function (req, res) {
  connection.query(
    "SELECT id,email,name,surname,role,deal,signature FROM users",
    function (err, rows) {
      if (err) res.json(err);
      res.json(rows);
    }
  ...
  if (
    ...
    jwt.verify(
      ...
      function (err, decode) {
        ...
        connection.query(
          "select id,email,role,signature from users where email='" + decode.email + "'",
          function (err, rows) {
            if (err) {
...
const passwordReset = function (req, res) {
  ...
  connection.query(
    ...
    function (err, rows) {
      ...
      if (user) {
        connection.query(
          "select * from authresets where user='" + user.id + "' AND status=0",
          function (err, row) {
            const authreset = row[0];
            console.log(row);
            if (authreset)
              connection.query(
                "DELETE FROM authresets where id='" + authreset.id + "'",
                function (err, rowdel) {
                  if (err) res.json({ err: err });
                }
              );
            ...
            bcrypt.hash(token, 10, function (err, hash) {
              ...
              const expire = date.datetimeParam(
                new Date(timeNow.setDate(timeNow.getDate() + 1))
              );
              console.log(expire);
              connection.query(
                `INSERT INTO authresets(user,token,expire) VALUES(${user.id},"${hash}",'${expire}')`,
                function (err, authresetinsert) {
                  if (err) res.json({ err: err });
                ...
                const transporter = nodemailer.createTransport(mailConfig.config);
                  const mailOptions = {
                    ...
                    html:
                      "<p>Witam,</p>" +
                      "<p>link do resetu hasła: </p>" +
                      "<a href=" +
                      "http://timesheet-klaster.herokuapp.com/" +
                      "reset/" +
                      user.id +
                      "/" +
                      token +
...
const storePassword = function (req, res) {
  ...
  connection.query(
    "select * from authresets where user='" + user + "' AND status=0",
    function (err, row) {
      ...
      bcrypt.compare(token, authreset.token, function (errBcrypt, resBcrypt) {
        ...
        bcrypt.hash(password, 10, function (err, hash) {
          if (err) res.json({ success: false, err: err });
          connection.query(
            `UPDATE users SET password='${hash}' WHERE id='${user}'`,
            function (err, rows) {
              if (err) res.json({ success: false, err: err });
              connection.query(
                `UPDATE authresets SET status=1 WHERE id='${authreset.id}'`,
                function (err, rows) {
